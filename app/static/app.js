...
function applyTheme(pref){const media=matchMedia('(prefers-color-scheme: dark)');const effective=pref==='system'?(media.matches?'dark':'light'):pref;document.documentElement.dataset.theme=effective;document.querySelector('meta[name="theme-color"]').content=effective==='dark'?'#171613':'#f4efe6';$('#themeQuick')&&($('#themeQuick').textContent=effective==='dark'?'☀':'☾')}
matchMedia('(prefers-color-scheme: dark)').addEventListener?.('change',()=>{if(S.user?.theme_pref==='system')applyTheme('system')});
async function refreshStatus(){const st=await fetch('/api/status',{cache:'no-store'}).then(r=>r.json());S.csrf=st.csrf;S.user=st.user;S.appVersion=st.app_version||'unknown';applyBranding(st.branding);$$('[data-app-version]').forEach(x=>x.textContent=`v${S.appVersion}`);if(S.user)applyTheme(S.user.theme_pref||'system');return st}
async function boot(){const st=await refreshStatus();if(st.setup_required){$('#authScreen').classList.remove('hidden');$('#setupForm').classList.remove('hidden');return}if(!st.user){$('#authScreen').classList.remove('hidden');const invite=new URLSearchParams(location.search).get('invite');if(invite){$('#registerForm').classList.remove('hidden');$('#registerForm').elements.invite_code.value=invite}else $('#loginForm').classList.remove('hidden');return}showShell()}
async function authSubmit(form,url){try{await api(url,jsonOpts('POST',formObj(form)));await refreshStatus();showShell()}catch(e){toast(e.message)}}
...
function recipeQuery(){const p=new URLSearchParams();const vals={q:$('#searchInput').value,category:S.category,owner:$('#ownerFilter').value,ingredients:$('#includeIngredient').value,exclude:$('#excludeIngredient').value,sort:$('#sortFilter').value};Object.entries(vals).forEach(([k,v])=>v&&p.set(k,v));return p.toString()}
async function loadRecipes(){S.recipes=await api('/api/recipes?'+recipeQuery());renderRecipes();$('#parentRecipeSelect').innerHTML=`<option value="">Not a variation</option>`+S.recipes.map(r=>`<option value="${r.id}">${esc(r.title)}</option>`).join('')}
function recipeCard(r){return `<article class="recipe-card" data-recipe="${r.id}"><div class="card-img">${r.cover_image?`<img src="/uploads/${esc(r.cover_image)}">`:`<span class="card-icon">${ICONS[r.category]||'🥄'}</span>`}</div><div class="card-body"><span class="card-kicker">${esc(r.category)} · ${esc(r.subcategory||'Recipe')}</span><h3>${esc(r.title)}</h3><div class="byline">${avatarHtml(r.owner_avatar,r.owner_name||r.legacy_contributor)}<span>by ${esc(r.owner_name||r.legacy_contributor||'Family')}</span></div><div class="card-bottom"><span class="rating">${r.rating_count?`★ ${Number(r.rating_avg).toFixed(1)} (${r.rating_count})`:'☆ Not rated'}</span><button class="fav ${r.is_favorite?'on':''}" data-fav="${r.id}">${r.is_favorite?'♥':'♡'}</button></div></div></article>`}
function renderRecipes(){$('#recipeCount').textContent=S.recipes.length;$('#resultMeta').textContent=`${S.recipes.length} recipe${S.recipes.length===1?'':'s'}`;$('#recipeGrid').innerHTML=S.recipes.length?S.recipes.map(recipeCard).join(''):`<div class="empty"><div>🍽</div><h3>No matches</h3><p>Try another filter or add a family recipe.</p></div>`;$$('[data-recipe]').forEach(el=>el.onclick=e=>{if(!e.target.closest('[data-fav]'))openRecipe(Number(el.dataset.recipe))});$$('[data-fav]').forEach(b=>b.onclick=async e=>{e.stopPropagation();await api(`/api/recipes/${b.dataset.fav}/favorite`,jsonOpts('POST',{}));loadRecipes()})}
let searchDebounce;['searchInput','includeIngredient','excludeIngredient'].forEach(id=>$('#'+id).oninput=()=>{clearTimeout(searchDebounce);searchDebounce=setTimeout(loadRecipes,250)});['ownerFilter','sortFilter'].forEach(id=>$('#'+id).onchange=loadRecipes);$('#clearFilters').onclick=()=>{$('#searchInput').value='';$('#includeIngredient').value='';$('#excludeIngredient').value='';$('#ownerFilter').value='';$('#sortFilter').value='az';S.category='';$$('[data-cat]').forEach(x=>x.classList.toggle('active',x.dataset.cat===''));$('#recipesHeading').textContent='All Recipes';loadRecipes()};

// ---------- Recipe editor ----------
let ingCounter=0;
function addIngredientRow(d={}){const key='i'+(++ingCounter);const row=document.createElement('div');row.className='ingredient-row';row.dataset.key=key;row.innerHTML=`<input class="ing-qty" placeholder="1½" value="${esc(d.quantity_text||d.quantity_num||'')}"><input class="ing-unit" placeholder="cup" value="${esc(d.unit||'')}"><input class="ing-name" placeholder="Ingredient" value="${esc(d.ingredient||'')}"><input class="ing-prep" placeholder="diced, divided…" value="${esc(d.prep_note||d.note||'')}"><input class="ing-aisle" placeholder="Produce" value="${esc(d.aisle||'')}"><button type="button" class="remove-row">×</button><div class="ing-flags"><label><input class="ing-optional" type="checkbox" ${d.optional?'checked':''}> Optional</label><label><input class="ing-scalable" type="checkbox" ${d.scalable===0||d.scalable===false?'':'checked'}> Scale quantity</label></div>`;row.querySelector('.remove-row').onclick=()=>{row.remove();refreshAllStepSelectors()};row.querySelector('.ing-name').addEventListener('input',()=>{clearTimeout(row._t);row._t=setTimeout(refreshAllStepSelectors,180)});$('#ingredientRows').appendChild(row);return row}
function currentIngredients(){return $$('.ingredient-row').map((r,i)=>({key:r.dataset.key,index:i,name:r.querySelector('.ing-name').value.trim()}))}
function addStepRow(d={}){const row=document.createElement('div');row.className='step-row';row.dataset.initialLinks=JSON.stringify(d.ingredients||[]);row.innerHTML=`<div class="step-top"><span class="step-number"></span><input class="step-title" placeholder="Step title (optional)" value="${esc(d.title||'')}"><input class="step-temp" type="number" placeholder="°F" value="${esc(d.temp_f||'')}"><input class="step-time" type="number" step="0.5" placeholder="min" value="${esc(d.duration_minutes||'')}"><button type="button" class="remove-row">×</button></div><textarea class="step-text" rows="3" placeholder="Be explicit: heat level, temperature, time, and what done looks like.">${esc(d.instruction||'')}</textarea><div class="step-meta"><input class="step-doneness" placeholder="Doneness cue: browned, 165°F internal…" value="${esc(d.doneness||'')}"><input class="step-timer-label" placeholder="Timer label (optional)" value="${esc(d.timer_label||'')}"></div><div class="step-ingredients"><strong>Ingredients used in this step</strong><div class="step-ing-grid"></div></div>`;row.querySelector('.remove-row').onclick=()=>{row.remove();renumberSteps()};$('#stepRows').appendChild(row);renumberSteps();refreshStepSelector(row,d.ingredients||[]);return row}
function renumberSteps(){$$('.step-row').forEach((r,i)=>r.querySelector('.step-number').textContent=i+1)}
function captureStepSelections(row){return [...row.querySelectorAll('.step-ing-choice')].filter(x=>x.querySelector('input').checked).map(x=>({key:x.dataset.key,fraction:Number(x.querySelector('select').value)}))}
function refreshStepSelector(row,initial=null){const saved=initial?null:captureStepSelections(row);const grid=row.querySelector('.step-ing-grid');const ings=currentIngredients();const init=initial||[];grid.innerHTML=ings.map((ing,i)=>{const s=saved?.find(x=>x.key===ing.key);const ii=init.find(x=>Number(x.ingredient_index)===i);const checked=!!(s||ii);const frac=s?.fraction??ii?.quantity_fraction??1;return `<label class="step-ing-choice" data-key="${ing.key}"><input type="checkbox" ${checked?'checked':''}> <span>${esc(ing.name||`Ingredient ${i+1}`)}</span><select title="Portion used"><option value="1" ${frac==1?'selected':''}>all</option><option value="0.75" ${frac==.75?'selected':''}>¾</option><option value="0.5" ${frac==.5?'selected':''}>½</option><option value="0.25" ${frac==.25?'selected':''}>¼</option></select></label>`}).join('')}
function refreshAllStepSelectors(){$$('.step-row').forEach(r=>refreshStepSelector(r))}
...
function formatQty(n){if(n==null||!Number.isFinite(Number(n)))return'';n=Number(n);const whole=Math.floor(n+1e-8),frac=n-whole;let best=null;for(const [v,s] of FRACTIONS){if(Math.abs(frac-v)<.035){best=s;break}}if(best)return whole?`${whole} ${best}`:best;if(Math.abs(n-Math.round(n))<.02)return String(Math.round(n));return String(Math.round(n*100)/100)}
function scaledIng(i,f=S.detailScale,portion=1){const q=i.scalable!==0&&i.quantity_num!=null?Number(i.quantity_num)*f*portion:null;const qt=q!=null?formatQty(q):(i.quantity_text||'');return [qt,i.unit,i.ingredient,i.prep_note?`(${i.prep_note})`:'',i.optional?'(optional)':''].filter(Boolean).join(' ')}
...
// Cook Mode: next/previous steps, screen wake lock, timers. Browsers cannot force device brightness.
async function requestWake(){if(!S.cook?.wakeWanted)return;try{if('wakeLock' in navigator){S.cook.wake=await navigator.wakeLock.request('screen');renderCookWakeStatus()}else throw new Error('Wake Lock not supported')}catch(e){S.cook.wake=null;renderCookWakeStatus(e.message)}}
async function releaseWake(){try{await S.cook?.wake?.release()}catch{}if(S.cook)S.cook.wake=null}
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&S.cook?.wakeWanted&&!S.cook.wake)requestWake()});
function startCookMode(){const r=S.currentRecipe;if(!r.steps?.length){toast('This legacy recipe needs structured steps before Cook Mode can guide it step by step.');return}S.cook={recipe:r,index:0,scale:S.detailScale,wake:null,wakeWanted:true,timer:null,timerEnd:null};$('#cookDialog').classList.toggle('dark-cook',document.documentElement.dataset.theme==='dark');$('#cookDialog').showModal();requestWake();renderCookMode()}
function renderCookWakeStatus(err=''){const el=$('#wakeStatus');if(!el)return;el.className='wake-status '+(S.cook?.wake?'on':'');el.textContent=S.cook?.wake?'☀ Screen awake':(err||'Screen wake unavailable')}
function renderCookMode(){const c=S.cook;if(!c)return;const r=c.recipe,s=r.steps[c.index];$('#cookMode').innerHTML=`<div class="cook-shell"><header class="cook-top"><button class="cook-close" id="endCook">× End</button><div class="cook-title">${esc(r.title)}</div><button class="wake-status" id="wakeStatus">Screen awake</button></header><main class="cook-body"><article class="cook-step"><div class="cook-progress">Step ${c.index+1} of ${r.steps.length} · ${Math.round(((c.index+1)/r.steps.length)*100)}%</div><h1>${esc(s.title||`Step ${c.index+1}`)}</h1>${s.ingredients?.length?`<div class="cook-use"><h3>Use in this step</h3>${s.ingredients.map(i=>`<div>${esc(scaledIng(i,c.scale,Number(i.quantity_fraction||1)))}</div>`).join('')}</div>`:''}<div class="cook-instruction">${esc(s.instruction)}</div><div class="cook-facts">${s.temp_f?`<span class="cook-fact">🌡 ${formatQty(s.temp_f)}°F</span>`:''}${s.duration_minutes?`<button class="cook-fact" id="startTimer">⏱ ${formatQty(s.duration_minutes)} min timer</button>`:''}${s.doneness?`<span class="cook-fact">✓ ${esc(s.doneness)}</span>`:''}<span id="timerLive" class="timer-live"></span></div></article></main><footer class="cook-nav"><button class="cook-close" id="prevStep" ${c.index===0?'disabled':''}>← Previous</button><span class="muted">Serves ${r.servings_base?formatQty(Number(r.servings_base)*c.scale):esc(r.servings_label||'')}</span><button class="primary" id="nextStep">${c.index===r.steps.length-1?'Finish ✓':'Next →'}</button></footer></div>`;renderCookWakeStatus();$('#endCook').onclick=endCookMode;$('#wakeStatus').onclick=async()=>{c.wakeWanted=!c.wakeWanted;if(c.wakeWanted)await requestWake();else await releaseWake();renderCookWakeStatus()};$('#prevStep').onclick=()=>{c.index=Math.max(0,c.index-1);clearCookTimer();renderCookMode()};$('#nextStep').onclick=async()=>{if(c.index===r.steps.length-1){await api(`/api/recipes/${r.id}/made`,jsonOpts('POST',{}));toast('Finished! Marked as made.');endCookMode();loadRecipes()}else{c.index++;clearCookTimer();renderCookMode()}};$('#startTimer')&&($('#startTimer').onclick=()=>startCookTimer(Number(s.duration_minutes)*60))}
function startCookTimer(seconds){clearCookTimer();S.cook.timerEnd=Date.now()+seconds*1000;S.cook.timer=setInterval(()=>{const left=Math.max(0,Math.ceil((S.cook.timerEnd-Date.now())/1000)),m=Math.floor(left/60),s=left%60;const el=$('#timerLive');if(el)el.textContent=`${m}:${String(s).padStart(2,'0')}`;if(left<=0){clearCookTimer();try{navigator.vibrate?.([250,120,250])}catch{}beep();toast('Timer finished')}} ,250)}
function clearCookTimer(){if(S.cook?.timer)clearInterval(S.cook.timer);if(S.cook){S.cook.timer=null;S.cook.timerEnd=null}}
function beep(){try{const A=window.AudioContext||window.webkitAudioContext,c=new A(),o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.frequency.value=880;g.gain.value=.12;o.start();setTimeout(()=>{o.stop();c.close()},650)}catch{}}
async function endCookMode(){clearCookTimer();await releaseWake();S.cook=null;$('#cookDialog').close()}

// ---------- Shopping lists ----------
async function loadLists(){if(!S.user)return;S.lists=await api('/api/shopping-lists');$('#cartCount').textContent=S.lists.reduce((n,l)=>n+(l.open_items||0),0);$('#shoppingLists').innerHTML=S.lists.map(l=>`<button class="list-card ${S.activeList===l.id?'active':''}" data-list="${l.id}"><strong>${esc(l.name)}</strong><span>${l.open_items} open · ${l.shared?'Shared':'Personal'}</span></button>`).join('');$$('[data-list]').forEach(b=>b.onclick=()=>openList(Number(b.dataset.list)))}
async function openList(id){S.activeList=id;const data=await api(`/api/shopping-lists/${id}`);$('#shoppingEmpty').classList.add('hidden');$('#shoppingContent').classList.remove('hidden');$('#shoppingTitle').textContent=data.list.name;const open=data.items.filter(x=>!x.checked).length;$('#shoppingMeta').textContent=`${open} item${open===1?'':'s'} left`;const groups={};data.items.forEach(i=>(groups[i.aisle||'Other']??=[]).push(i));$('#shoppingItems').innerHTML=Object.entries(groups).map(([aisle,items])=>`<div class="shopping-group"><h3>${esc(aisle)}</h3>${items.map(i=>`<div class="shop-item ${i.checked?'checked':''}"><input type="checkbox" data-check-item="${i.id}" ${i.checked?'checked':''}><div><span class="shop-qty">${esc([i.quantity_text,i.unit].filter(Boolean).join(' '))}</span> <span class="shop-name">${esc(i.display_name)}</span>${i.source_recipe_name?`<small class="muted"> · ${esc(i.source_recipe_name)}</small>`:''}</div><button class="trash" data-delete-item="${i.id}">×</button></div>`).join('')}</div>`).join('');$$('[data-check-item]').forEach(c=>c.onchange=async()=>{await api(`/api/shopping-items/${c.dataset.checkItem}`,jsonOpts('PATCH',{checked:c.checked}));await Promise.all([openList(id),loadLists()])});$$('[data-delete-item]').forEach(b=>b.onclick=async()=>{await api(`/api/shopping-items/${b.dataset.deleteItem}`,{method:'DELETE'});await Promise.all([openList(id),loadLists()])});await loadLists()}
$('#manualItemForm').onsubmit=async e=>{e.preventDefault();if(!S.activeList)return;await api(`/api/shopping-lists/${S.activeList}/items`,jsonOpts('POST',formObj(e.currentTarget)));e.currentTarget.reset();openList(S.activeList)};$('#clearChecked').onclick=async()=>{if(S.activeList){await api(`/api/shopping-lists/${S.activeList}/clear-checked`,jsonOpts('POST',{}));openList(S.activeList)}};$('#newListBtn').onclick=async()=>{const name=prompt('List name:');if(!name)return;const shared=confirm('Share this list with the family?');await api('/api/shopping-lists',jsonOpts('POST',{name,shared}));loadLists()};
async function openAddToList(items,title='Add to shopping list'){await loadLists();S.listItems=items;$('#listDialogTitle').textContent=title;$('#targetList').innerHTML=S.lists.map(l=>`<option value="${l.id}">${esc(l.name)}</option>`).join('');const main=items[0];$('#targetServings').value=main.servings||'';$('#addToListDialog').showModal()}
$('#confirmAddToList').onclick=async()=>{const lid=Number($('#targetList').value);try{for(let i=0;i<S.listItems.length;i++){const x=S.listItems[i];const servings=i===0?($('#targetServings').value||x.servings):x.servings;await api(`/api/shopping-lists/${lid}/from-recipe/${x.id}`,jsonOpts('POST',{servings,skip_optional:$('#skipOptional').checked}))}$('#addToListDialog').close();toast(S.listItems.length>1?'Entire meal added to list':'Ingredients added to list');loadLists()}catch(e){toast(e.message)}};

// Pantry
$('#pantryBtn').onclick=()=>$('#pantryDialog').showModal();$$('[data-close]').forEach(b=>b.onclick=()=>$('#'+b.dataset.close).close());$('#pantryForm').onsubmit=async e=>{e.preventDefault();const ingredients=$('#pantryInput').value.trim();if(!ingredients)return;const rows=await api('/api/discover?ingredients='+encodeURIComponent(ingredients));$('#pantryResults').innerHTML=rows.length?rows.map(r=>`<div class="match-card" data-pantry-recipe="${r.id}"><div class="match-top"><strong>${esc(r.title)}</strong><span class="match-percent">${r.match_percent}% match</span></div><div class="muted">Missing: ${esc(r.missing.join(', ')||'nothing essential')}</div></div>`).join(''):`<div class="empty"><p>No strong matches yet.</p></div>`;$$('[data-pantry-recipe]').forEach(x=>x.onclick=()=>{$('#pantryDialog').close();openRecipe(Number(x.dataset.pantryRecipe))})};

// Family / profile / activity
function renderFamily(){$('#familyGrid').innerHTML=S.users.map(u=>`<article class="person-card" data-person="${u.id}">${avatarHtml(u.avatar_path,u.display_name)}<h3>${esc(u.display_name)}</h3><span class="pill">${esc(u.role)}</span><p class="muted">${u.recipe_count} recipes · ${u.comment_count} comments</p></article>`).join('');$$('[data-person]').forEach(x=>x.onclick=()=>loadProfile(Number(x.dataset.person),true))}
async function loadProfile(uid,go=false){S.profileUid=uid;const d=await api(`/api/profile?user_id=${uid}`);$('#profileContent').innerHTML=`<div class="profile-hero panel">${avatarHtml(d.user.avatar_path,d.user.display_name,'avatar avatar-xl')}<h1>${esc(d.user.display_name)}</h1><span class="pill">${esc(d.user.role)}</span><div class="stats" style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:16px"><div class="pill">${d.stats.recipes} recipes</div><div class="pill">${d.stats.ratings} ratings</div><div class="pill">${d.stats.comments} comments</div><div class="pill">${d.stats.made_count} made</div></div></div><div class="section-head" style="margin-top:24px"><div><h2>Recipes added</h2></div></div><div class="recipe-grid">${d.recipes.map(r=>`<article class="recipe-card" data-profile-recipe="${r.id}"><div class="card-body"><span class="card-kicker">${esc(r.category)}</span><h3>${esc(r.title)}</h3></div></article>`).join('')}</div>`;$$('[data-profile-recipe]').forEach(x=>x.onclick=()=>openRecipe(Number(x.dataset.profileRecipe)));$('#profileSettings').classList.toggle('hidden',uid!==S.user.id);if(uid===S.user.id)$('#themeForm').elements.theme_pref.value=S.user.theme_pref||'system';if(go)nav('profile')}
$('#avatarForm').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);const r=await fetch('/api/me/avatar',{method:'POST',headers:{'X-CSRF-Token':S.csrf},body:fd});const d=await r.json();if(!r.ok){toast(d.error);return}await refreshStatus();showShell();toast('Profile photo updated');loadProfile(S.user.id)};$('#themeForm').onsubmit=async e=>{e.preventDefault();const pref=e.currentTarget.elements.theme_pref.value;const r=await api('/api/me/preferences',jsonOpts('PATCH',{theme_pref:pref}));S.user=r.user;applyTheme(pref);toast('Appearance saved')};$('#passwordForm').onsubmit=async e=>{e.preventDefault();try{await api('/api/change-password',jsonOpts('POST',formObj(e.currentTarget)));e.currentTarget.reset();toast('Password changed')}catch(err){toast(err.message)}};
